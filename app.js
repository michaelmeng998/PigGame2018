...
first create a array to store the legit scores, and create a variable to store a specific rounds values. Next implement a count function that chooses which player gets to roll the dice. Call a random # generator and return a randowm number from 1 to 6. add to the round var. If its a 1, make the stored round # a 0, and change the counter. Do this until one of the array values becomes greater than or equal to 100.
...
in css and html add the second die, then in javascript we need to call the html class dice2,= when the button is pressed and display exactly like how the first dice is. Then, in order to add up the scores, you need to use the same button roll function to add to the round score.

*/


var scores, roundscore, activeplayer, gameplaying, temp;
...
document.querySelector('.btn-hold').addEventListener('click', function(){
   ...
   //add current score to the global score
    ...
    var input = document.querySelector('.final-score').value;
        console.log("input winning score is: " + input);
